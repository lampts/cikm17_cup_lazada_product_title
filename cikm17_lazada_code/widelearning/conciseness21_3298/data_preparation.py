...
class DataPreparation(object):
    ...
    def __init__(self):
        ...
        if "x" not in self.stops:
            print("x is not a stop word, adding it")
            self.stops.add("x")
        '''
        self.stops = [
        ...
        self.measurement_units = ["g", "oz", "kg", "kgs", "cm", "mm", "ml", "mg", "tc", "gb", "tb", "mb", "mah", "mhz", "vac", "inch", "inches", "ft"]
    ...
    '''
    def generate_features(self, df_input, text_column, target_column, num_top_features, num_common_features, num_bottom_features, new_feature_prefix):
    ...
    def clean_data(self, target_column, verbose = 0):
        ...
        for cat in ["1"]:
            ...
            for i in range(number_of_cats):
                ...
                df_extra_title = self.generate_features(df_train, "title", target_column, num_top_features, num_common_features, num_bottom_features, "ct" + cat + "_")
    ...
    def add_w2v_data(self, target_column):

        ready_filename = os.path.join(self.HOME_DIR, "input", "data_ready_" + target_column + ".csv")
        df_ready = pd.read_csv(ready_filename)
        print("Add w2v data, df_ready {}".format(df_ready.shape))

        df_concat_list = [df_ready]
        for text_column in ["title", "desc"]:
            for cat_column in ["cat1", "cat2", "cat3"]:
                for train_value in ["1"]:
                    cat_filename = text_column + "." + cat_column + "." + target_column + ".predict.csv." + train_value
                    cat_filename = os.path.join(self.HOME_DIR, "output", cat_filename)
                    column_name = text_column + "_" + cat_column + "_match"
                    df_cat = pd.read_csv(cat_filename, usecols = [column_name])
                    new_column_name = column_name + "_" + train_value
                    df_cat.columns = [new_column_name]
                    df_concat_list.append(df_cat)

        df_result = pd.concat(df_concat_list, axis=1)
        df_result["title_sum_match_1"] = df_result["title_cat1_match_1"] + df_result["title_cat2_match_1"] + df_result["title_cat3_match_1"]
        df_result["desc_sum_match_1"] = df_result["desc_cat1_match_1"] + df_result["desc_cat2_match_1"] + df_result["desc_cat3_match_1"]
        result_filename = os.path.join(self.HOME_DIR, "input", "data_ready_w2v_" +target_column + ".csv")

        print("Df_result {}".format(len(df_result)))
...
if __name__ == '__main__':
    ...
    print("Test {}".format(dp.rectify_unit("Hello 100g 23oz")))
    if labels[0]:
